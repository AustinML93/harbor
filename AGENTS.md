...
**Use Python 3.11 for backend development.** The backend Docker image, GitHub Actions workflow, and `.python-version` file all target Python 3.11. Do not create the backend virtualenv with Python 3.14 unless you are intentionally doing compatibility work; the pinned SQLAlchemy dependency set currently has typing-introspection failures on 3.14.
